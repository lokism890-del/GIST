...
function ProfileDropdown({ user, isDarkMode, onSignOut }: { user: any, isDarkMode: boolean, onSignOut: () => void }) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        <span className={`relative z-10 transform flex items-center justify-center -rotate-45 font-bold text-[13px] tracking-widest ${isDarkMode ? 'text-cyan-400' : 'text-cyan-600'}`}>
          {userInitial}
        </span>
      </button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`absolute right-0 mt-5 w-64 backdrop-blur-xl border rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.2)] overflow-hidden z-50 ${
              isDarkMode ? 'bg-[#0A111F]/95 border-[#1E293B]' : 'bg-white/95 border-slate-200'
            }`}
          >
            {/* User Info Header */}
            <div className={`p-4 border-b ${isDarkMode ? 'border-[#1E293B] bg-[#0D1322]/50' : 'border-slate-100 bg-slate-50/50'}`}>
              <div className="flex items-center gap-3">
                <div className={`w-9 h-9 border rounded-lg transform rotate-45 flex items-center justify-center shrink-0 ${isDarkMode ? 'bg-[#121A2B] border-[#23314A]' : 'bg-white border-slate-200'}`}>
                  <span className={`transform -rotate-45 font-bold text-base ${isDarkMode ? 'text-cyan-400' : 'text-cyan-600'}`}>
                    {userInitial}
                  </span>
                </div>
                <div className="overflow-hidden">
                  <p className={`font-medium text-sm truncate ${isDarkMode ? 'text-white' : 'text-slate-800'}`} title={user.email}>
                    {user.email}
                  </p>
                  <p className={`text-[9px] font-bold uppercase tracking-widest mt-0.5 ${isDarkMode ? 'text-cyan-500/80' : 'text-cyan-600/80'}`}>
                    Free Plan
                  </p>
                </div>
              </div>
            ...
            <div className="p-2 space-y-1">
              <button className={`w-full flex items-center gap-3 px-3 py-2.5 text-xs rounded-xl transition-colors ${isDarkMode ? 'text-slate-300 hover:text-white hover:bg-[#121A2B]' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'}`}>
            ...
            {/* Sign Out Action */}
            <div className={`p-2 border-t ${isDarkMode ? 'border-[#1E293B]' : 'border-slate-100'}`}>
              <button
                onClick={onSignOut}
                className={`w-full flex items-center gap-3 px-3 py-2.5 text-xs rounded-xl transition-colors ${isDarkMode ? 'text-rose-400 hover:text-rose-300 hover:bg-rose-500/10' : 'text-rose-600 hover:text-rose-700 hover:bg-rose-50'}`}
              >
